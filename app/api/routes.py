...
async def orchestrate(request: OrchestratorRequest):
    ...
    # 2. Persistência: Salva a pergunta e busca o contexto atualizado (Tudo Async)
    await salvar_mensagem_historico(request.session_id, request.tenant_id, "user", request.query, request.supabase_url, request.supabase_key)
    historico = await buscar_contexto_conversa(request.session_id, request.tenant_id, request.supabase_url, request.supabase_key)
    ...
    if target_agent == "main":
        ...
        # Regra de Negócio: Controle de Acesso (RBAC)
    ...
    # 5. Persistência: Salva a resposta gerada
    await salvar_mensagem_historico(request.session_id, request.tenant_id, "assistant", answer, request.supabase_url, request.supabase_key)
